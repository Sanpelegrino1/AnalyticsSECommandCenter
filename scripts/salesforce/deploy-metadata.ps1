...
try {
    if ($Metadata -and $Metadata.Count -gt 0) {
        & sf project deploy start --target-org $TargetOrg --metadata $Metadata
        exit $LASTEXITCODE
    }

    & (Join-Path $PSScriptRoot '..\..\salesforce\scripts\deploy-manifest.ps1') -TargetOrg $TargetOrg -ManifestPath $ManifestPath
    exit $LASTEXITCODE
} finally {
    Pop-Location
}
